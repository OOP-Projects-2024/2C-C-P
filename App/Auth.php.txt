...
class Authentication{
    ...
    public function login($body){
        ...
        try{
           $sqlString = "SELECT enrolleeid, username, password, token FROM accounts_tbl WHERE username=?";
           $stmt = $this->pdo->prepare($sqlString);
           ...
           if($stmt->rowCount() > 0){
            $result = $stmt->fetchAll()[0];
                if($this->isSamePassword($password, $result['password'])){
                    ...
                    $payload = array("enrolleeid"=>$result['enrolleeid'], "username"=>$result['username'], "token"=>$token_arr[2]);
                }
                else{
                    $code = 401;
                    $payload = null;
                    ...
                    $token = $this->generateToken($result['enrolleeid'], $result['username']);
                    $this->saveToken($token, $result['username']);
                    $token_arr = explode('.', $token);
                    $this->saveToken($token_arr[2], $result['username']);
                    $payload = array("enrolleeid"=>$result['enrolleeid'], "username"=>$result['username'], "token"=>$token_arr[2]);
                }
    ...
    public function addAccount($body){
        ...
        try{
            $sqlString = "INSERT INTO accounts_tbl(enrolleeid, username, password) VALUES (?,?,?)";
            $sql = $this->pdo->prepare($sqlString);
            $sql->execute($values);
